<?php
//GuaranteeNationalCard
namespace controller;

use ControllerScheme;

class GuaranteeNationalCard extends ControllerScheme {
	const name = 'اطلاعات کارت ملی';
	...
	public function wizardAction(bool $isAdd = true) {
		...
		file_put_contents(__SOURCE__ . "images/GuaranteeNationalCard/$name", $image);
		$array = [
			'guarantee_documents_id'   => $data['guarantee_documents_id'],
			'national_card_image'      => $name,
			'national_card_number'     => $data['national_card_number'],
			'national_card_expiration' => $data['national_card_expiration'],
			'national_card_status'     => $data['national_card_status'],
			'national_card_desc'       => $data['national_card_dsec'],
		];
		if ($isAdd) {
